...
class MultiTaskDnn(nn.Module):
    def __init__(self, ae_model_path):
        ...
        self.multi_head_dict = nn.ModuleDict({str(clst): copy.deepcopy(self.head_pattern) for clst in range(15)})
    ...
    def forward(self, x):
        ...
        """
        head_res = {}
        for clst in range(15):
            x_bar, tra1, tra2, tra3, z = self.ae(x[clst])
            _x = self.body(x_bar)
...
def my_task(ae_path):
    ...
    PREFIX = os.path.splitext(os.path.basename(ae_path))[0]
    ...
    clst_recorder = {f'{des}_{tp}_{mtc}': [] for des, tp, mtc in
                     product(range(15), ['train', 'val'], ['loss', 'rec', 'acc'])}
    ...
    for epoch in range(EPOCHS):
        ...
        for mode in ["train", "val"]:
            ...
            dse_t_metrics = {clst: MetricsRecoder() for clst in range(15)}
            ...
            dse_res_dict = {clst: dse_t_metrics[clst].get_metrics() for clst in range(15)}
        ...
        if epoch % 10 == 0 and os.path.exists(MODEL_FILE_NAME):
            ...
            test_model = MultiTaskDnn(ae_path)
            test_model.load_state_dict(torch.load(MODEL_FILE_NAME))
            test_model.to(device)
            test_model.eval()

            metrics_dict = {str(clst): TestMetricsRecoder() for clst in range(15)}
